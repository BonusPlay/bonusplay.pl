<template>
	<v-layout align-space-around justify-space-around column text-xs-justify>
		...
		<v-flex>
			...
			In this post, we'll only talk about #1 and #2. Generally speaking, external cheats are easier to write (since they ususally use simple OS api to interact with game's memory),
			but don't offer as much flexibility as internal, which can e.g. hook functions.
			<!-- External -->
			<h3 class="display-1">External</h3>
			The easier way (and also, the way I started).
			I used
			...
			and a simple
			<a href><code>OpenProcess</code></a>
			with <code>PROCESS_ALL_ACCESS</code> to obtain process to handle. Some anticheats detect this as cheat, but most don't, since that would
			...
			<a href="https://docs.microsoft.com/en-us/windows/desktop/api/memoryapi/nf-memoryapi-readprocessmemory">
				<code>ReadProcessMemory</code>
			</a>
			and
			<a href="https://docs.microsoft.com/en-us/windows/desktop/api/memoryapi/nf-memoryapi-writeprocessmemory">
			...
			<!-- VFT -->
			<h4 class="headline">VFT</h4>
			This is the hard way, but it also allows to have more fun. First, you need to reverse engineer some part of code, you want to edit.
			...
			<ul>
				<li>
					...
					<a href="https://docs.microsoft.com/en-us/windows/desktop/api/processthreadsapi/nf-processthreadsapi-createremotethread">
						<code>CreateRemoteThread</code>
					</a> and
					<a href="https://docs.microsoft.com/en-us/windows/desktop/api/libloaderapi/nf-libloaderapi-loadlibrarya">
		...
		<v-flex>
			<v-layout wrap fill-height align-center>
				...
				<v-flex xs12 md11>
					<h2 class="display-2 text-xs-center">The cheat itself</h2>
