<template>
	<v-layout column text-xs-justify>
		<v-flex>
			<!-- Intro -->
			<h1 class="display-2 text-xs-center">BonusPlay.pl</h1>In this blog post, I'm going to describe the architecture of this very website.
		...
		<!-- Frontend -->
		<v-flex>
			<h1 class="display-1 text-xs-center">Frontend</h1>Frontend is written in Vue. The reasony I prefer Vue over React is that it separates all 3
		...
		<!-- Backend -->
		<v-flex>
			<h2 class="display-2 text-xs-center">Backend</h2>Here comes the more interesting stuff. I wanted high performance, low cost backend.
			Why?
			...
			<v-layout wrap fill-height align-center>
				...
				<v-flex xs12 md11>
					<h4 class="headline">Reverse Proxy</h4>
					...
					from going outside, but I can connect to it with a simple ssh tunnel (<code>ssh -nNT -L port:localhost:port user@domain</code>).
			...
			<v-btn to="/blog/random/mail-server" rounded>
				<v-icon left>fas fa-link</v-icon>
				Mail-server
			</v-btn>
		</v-flex>
		...
		<!-- deployment -->
		<v-flex>
			<h1 class="display-1 text-xs-center">Deployment</h1>
			I have separate post about my way of deploying website, which you can read here:
			<v-btn to="/blog/random/git-deploy" rounded>
		...
		<!-- Afterword -->
		<v-flex>
			<h1 class="display-1 text-xs-center">Afterword</h1>
