<template>
	<div id="content">
		<section class="section">
			<div class="columns container is-fluid">
				...
				<div class="column is-3 is-center">
					<figure class="image">
						<img id="me" :src="require('@/assets/mecrop.webp')">
					</figure>
				</div>
			</div>
		</section>
		<section class="section">
			...
			<div class="columns container is-fluid">
				<div v-for="item in projects" :key="item.title" class="column is-4 has-text-justified">
					<h1 class="title is-2 has-text-centered">{{ item.title }}</h1>
					<p>{{ item.description }}</p>
					<div>
						<a v-for="link in item.links" :key="link.to" :href="link.to" class="button is-dark is-spaced">
